...
const char chntpw_version[] = "chntpw version 0.99.5 070923 (decade), (c) Petter N Hagen";
...
void recoveryconsole()
{
  ...
  cmd = get_dword(hive[H_SOF],0,scpath);
  sec = get_dword(hive[H_SOF],0,slpath);
  ...
  if (*yn == 'y') {
    cmd ^= 1;
    sec ^= 1;
    if (!put_dword(hive[0], 0, scpath, cmd)) printf("Update of SET level failed registry edit\n");
